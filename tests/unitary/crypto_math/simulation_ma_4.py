...
def inv_target(A, gamma, x, D):
    ...
    if gamma > 0:
        K = gamma**2 * K0 / (gamma + 10**18*(1. - K0))**2
    K *= A
...
class Trader:
    def __init__(self, A, gamma, D, n, p0, mid_fee=1e-3, out_fee=3e-3, price_threshold=0.01, fee_gamma=None, log=True):
        self.p0 = p0
    ...
    def price(self, i, j):
        ...
        return dx_raw / (self.curve.x[j] - self.curve.y(self.curve.x[i] + dx_raw, i, j))
    ...
    def simulate(self, mdata):
        ...
        for i, d in enumerate(mdata):
            ...
            candle = min(max(self.mid_fee, abs((d['high'] - d['low']) / d['high'])), 0.1)
            ...
            if self.log:
                try:
                    print("{0}\ttrades: {1}\tAMM: {2:.6f}\tTarget: {3:.6f}\tVol: {4:.4f}\tPR: {5: .2f}\txCP-growth: {6:.5f}\tAPY:{7:.1f}%\tfee:{8:.3f}%".format(
                          i, ctr, last, self.curve.p[1] / 1e18, self.total_vol/1e18, (self.xcp_profit_real - 1)/(self.xcp_profit - 1),
                          self.xcp_profit_real,
                          (self.xcp_profit_real ** (86400 * 365 / (d['t'] - mdata[0]['t'] + 1)) - 1) * 100,
                          self.fee() * 100))
                except Exception:
                    pass
...
if __name__ == '__main__':
    ...
    trader = Trader(100, 1.5e-4, 10**18, 2, test_data[0]['close'], mid_fee=0.7e-3, out_fee=4.0e-3,
                    price_threshold=0.004, fee_gamma=0.01)
